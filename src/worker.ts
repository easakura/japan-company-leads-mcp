/**
 * gbiz-leads-mcp リモート版（Cloudflare Workers）
 * シークレット GBIZINFO_API_TOKEN が必要（wrangler secret put GBIZINFO_API_TOKEN）
 */
import { McpAgent } from "agents/mcp";
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { registerTools, SERVER_INFO } from "./core.js";

export class GbizLeadsMCP extends McpAgent {
  server = new McpServer(SERVER_INFO);

  async init() {
    registerTools(this.server, (this.env as any)?.GBIZINFO_API_TOKEN ?? "");
  }
}
...
export default {
  async fetch(request: Request, env: any, ctx: unknown) {
    ...
    if (pathname === "/mcp" || pathname.startsWith("/mcp/")) {
      return GbizLeadsMCP.serve("/mcp").fetch(request as any, env as any, ctx as any);
    }
    if (pathname === "/sse" || pathname.startsWith("/sse/")) {
      return GbizLeadsMCP.serveSSE("/sse").fetch(request as any, env as any, ctx as any);
    }
    return new Response(
